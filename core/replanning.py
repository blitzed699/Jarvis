...
class ReplanningEngine:
    ...
    def _infer_root_cause(
        ...
        mapping = {
            FailureCategory.TOOL_ERROR:
                "Tool execution failed — path, command, or dependency issue.",
            FailureCategory.AGENT_ERROR:
                "Agent produced invalid or incomplete output.",
            FailureCategory.PERMISSION_DENIED:
                "Insufficient permissions for the requested operation.",
            FailureCategory.TIMEOUT:
                "Operation exceeded time limit.",
            FailureCategory.HALLUCINATION:
                "Agent claimed success but evidence is missing.",
            FailureCategory.VALIDATION_FAILED:
                "Output did not match expected structure or values.",
            FailureCategory.DEPENDENCY_MISSING:
                "Required prerequisite step was not completed.",
            FailureCategory.UNKNOWN:
    ...
    def _suggest_fix(
        ...
        fixes = {
            FailureCategory.TOOL_ERROR:
                "Verify paths and commands exist before execution.",
            FailureCategory.AGENT_ERROR:
                "Provide additional context and retry with clearer instructions.",
            FailureCategory.PERMISSION_DENIED:
                "Request elevated permissions or change target path.",
            FailureCategory.TIMEOUT:
                "Increase timeout or break task into smaller subtasks.",
            FailureCategory.HALLUCINATION:
                "Force re-execution with explicit verification checkpoints.",
            FailureCategory.VALIDATION_FAILED:
                "Adjust parameters to match expected output format.",
            FailureCategory.DEPENDENCY_MISSING:
                "Ensure prerequisite steps complete successfully first.",
            FailureCategory.UNKNOWN:
                "Gather more diagnostic information before retrying.",
        }
    ...
    def generate_recovery_plan(
        ...
        if strategy == RecoveryStrategy.SUBSTITUTE_AGENT:
            ...
            new_step = {
                ...
                "agent": alt_agent,
                "depends_on": (
                    failed_step.depends_on
                    if hasattr(failed_step, 'depends_on')
                    else []
                )
            }
    ...
    def _get_world_state_context(self) -> str:
        """
        Return a compact snapshot of the current WorldState for replanning.

        WorldState is authoritative when available, but replanning must remain
        compatible with lightweight test doubles and older state objects.
        """
        if not self.world_state:
            return "WorldState unavailable."

        try:
            get_summary = getattr(self.world_state, "get_state_summary", None)

            if callable(get_summary):
                try:
                    summary = get_summary(verbose=True)
                except TypeError:
                    summary = get_summary()
            ...
            state_dict = getattr(self.world_state, "to_dict", None)
        ...
        return "WorldState available but no readable summary could be generated."
    ...
    def _llm_replan(
        ...
        """Use LLM to generate recovery steps using the current WorldState."""
        if not self.llm:
            return []
        ...
        world_state_context = self._get_world_state_context()

        prompt = f"""You are JARVIS's replanning system. A plan step failed and needs recovery.

You must reason from the CURRENT WORLD STATE, not only from the original plan.
The WorldState is the authoritative record of what JARVIS currently knows about
the environment, previous actions, failures, corrections, and active plan.
...
Recovery strategy: {strategy.value}

Remaining steps before failure:
{remaining_desc}

Generate a JSON array of steps to recover and complete the goal.
Use the current WorldState to avoid repeating actions that are already known
to have failed, already completed, or already been corrected.
...
Return ONLY raw JSON array. No markdown."""
        ...
        return [{
            "id": failed_step.id,
            "description": (
                f"[RECOVERED] {failed_step.description} — "
    ...
    def execute_recovery(
        ...
        remaining_steps: List,
        attempt_count: int = 1
    ) -> Tuple[RecoveryPlan, FailureAnalysis]:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for h in self.replan_history:
            cats[h["analysis"]["category"]] = (
                cats.get(h["analysis"]["category"], 0) + 1
            )
            strats[h["strategy"]] = (
                strats.get(h["strategy"], 0) + 1
            )
